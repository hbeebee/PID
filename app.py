...
def _sw(): st.session_state.mk=st.session_state._mr
opts=list(MODEL_CLASSES.keys())
labels=[]
for k in opts:
    m=models.get(k)
    r2s=(f"  R²={m.r2:.4f}" if m and not m.error and m.r2 else "  (н/д)")
    sfx="  ✓" if k==best_key else ""
    labels.append(f"{MODEL_LABELS[k]}{sfx}{r2s}")

st.radio("Модель:",opts,format_func=lambda k:labels[opts.index(k)],
         index=opts.index(st.session_state.mk),key="_mr",on_change=_sw,horizontal=True)
...
if dm and dm.error:
    ...
    mc[len(params)].metric("R²",f"{dm.r2:.4f}" if dm.r2 else "—")
    ...
    with st.expander(f"📊 Детали по {n_steps} ступенькам"):
        ...
        for s in steps:
            ...
            row["R² сегм."]=f"{r2s:.4f}"; row["RMSE сегм."]=f"{rmses:.4f}"
            rows.append(row)
...
for key in MODEL_CLASSES:
    ...
    if m and not m.error:
        ...
        row.update({"R²":f"{m.r2:.4f}" if m.r2 else "—",
                    "RMSE":f"{m.rmse:.4f}" if m.rmse else "—",
                    "Параметры":ps,"G(s)":m.tf_string()})
    else:
        row.update({"R²":"—","RMSE":"—","Параметры":m.error if m else "—","G(s)":"—"})
    cmp.append(row)
...
if not bm or bm.error:
    ...
    if pid_rec.get("Derivative (Td)") is not None:
        pc[2].metric("Deriv Td",f"{pid_rec['Derivative (Td)']:.4f}")
...
| | Формула | Значение |
|--|---------|---------|
| **Kc** | `(2T+d)/(K·(2e+d))` | **{pid_rec['Gain (Kc)']:.4f}** |
| **Ti** | `T+d/2` | **{pid_rec['Reset (Ti)']:.4f}** |
| **Td** | `T·d/(2T+d)` | **{pid_rec.get('Derivative (Td)') or '—'}** |

K={K_p:.4f} · T={T_p:.4f} · d={d_p:.4f} · e={pid_rec['e']:.4f} · ступенек: {n_good}/{n_steps}
""")
    ...
    if any([kc_ov!=0,ti_ov!=0,td_ov!=0]):
        ...
        ac[1].metric("Ti",f"{Ti_u:.4f}",delta=f"{Ti_u-pid_rec['Reset (Ti)']:+.4f}")
        if pid_type=="PID":
            ac[2].metric("Td",f"{Td_u:.4f}",delta=f"{Td_u-(pid_rec.get('Derivative (Td)') or 0):+.4f}")
